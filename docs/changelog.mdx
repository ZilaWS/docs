...
- The serverside can now set and remove cookies.
  - The serverside does not have access to cookies that were created after the establishment of the ws connection by default. In order to resync the cookies, use the `syncCookies` function. This also means that it's important to sync the cookies after one got deleted.
  - New local events:
    - onCookieSet
    - onCookieDelete