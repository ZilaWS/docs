...
## Asking for data
To ask the server for data, the `waiter` function can be used. This will **trigger a server side [MessageHandler](/docs/intro#recieving-data-from-the-client-side)** and get its return value.
```ts
