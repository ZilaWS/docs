...
## Recieving data from the clientside
In order to recieve data from the clientside, you need to register a `MessageHandler`. A `MessageHandler` has an identifier and a callback with any number of any typed parameters. The callback gets called when a client asks for it. The data recieved from the client will be passed as parameters.
